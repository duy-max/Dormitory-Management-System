...
import org.view.dialog.DepartmentDialog;
...
public class DepartmentPanel extends JPanel implements Panel {
 ...
 public void resizeColumnTable() {
    ...
    public void reloadTable() {
        DepartmentController.initialData();
        DepartmentController.initialModel();
        viewTable.setModel(DepartmentController.getModel());
        resizeColumnTable();
    }

    @Override
    public void actionAdd() {
        new DepartmentDialog(null).setVisible(true);
    }
    ...
    public void actionEdit() {
        ...
        new DepartmentDialog(DepartmentController.showFullInfo(t)).setVisible(true);
    }
    ...
    public void actionDelete() {
        ...
        for (int i = 0; i < viewTable.getRowCount(); i++) {
            if ("true".equals(viewTable.getValueAt(i, 0).toString().trim())) {
                ++count;
                arr.add(new String[]{
                    viewTable.getValueAt(i, 1).toString(),
                    viewTable.getValueAt(i, 2).toString()
                });
            }
        }
        if (count == 0) {
        ...
        count = 0;
        for (String[] s : arr) {
            if (DepartmentController.delData(s[0])) {
                ++count;
                // Thu hồi quyền của quản lí cũ
                DepartmentController.Revoke(s[1]);
            }
        }
        MetroUI.notificate(
            "Thông báo",
            "Đã xóa " + count + " phòng ban",
...
//        new DepartmentSearchDialog().setVisible(true);
    }
    ...
    private void initComponents() {
        ...
        lbTitle.setText("Quản Lý Phòng Ban");
        ...
        txtSearch.setText("Nhập mã phòng ban");
